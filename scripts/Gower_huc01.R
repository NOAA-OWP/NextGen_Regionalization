    ...
    while(sum(recs0 %in% recs1)!=length(recs0)) {
      ...
      for (rec1 in recs) {
        ...
        if (length(donor1)==0 & run1=="base") {
          ...
          for (att1 in attrs0) {
            # there was a bug in the line below (hence the there initial HUC-01 runs were impacted) 
            ...
            ix1 <- which(abs(dtAttrAll[[att1]][match(rec1,dtAttrAll$id)] - dtAttrAll[[att1]][match(donor0,dtAttrAll$id)]) <= threshs0[match(att1,attrs0)])
