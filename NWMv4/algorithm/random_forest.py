def func(config, dtAttrAll,scenario, dist_spatial):
    ...
    for run1 in attrs1: # attr round
        ...
        while len([x for x in recs0 if x in recs1]) != len(recs0):
            ...
            myscores = dtAttr.drop(config['non_attr_cols'])
            rf1 = urf(n_trees=config['pars']['random_forest']['nTrees'], max_depth=config['pars']['random_forest']['maxDepth'])
            ...
            distAttr0.columns = donorsAll1.append(receiversAll1)
            distAttr0.index = donorsAll1.append(receiversAll1)
            
            distAttr0 = distAttr0[donorsAll1]
            distAttr0 = distAttr0.iloc[receiversAll1]
