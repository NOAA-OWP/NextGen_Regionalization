...
layer_exists <- function(gpkg, layer) {
  return(layer %in% st_layers(gpkg)$name)
}
...
subset_network = function(gpkg,
  ...
  if (layer_exists(gpkg, "crosswalk")) {
    #TODO: GROSS!!!!
    ll[['crosswalk']]     = tryCatch({
      filter(read_sf(gpkg,  "crosswalk"), id %in% ids) }, 
      error = function(e){
        filter(read_sf(gpkg,  "crosswalk"), aggregated_ID %in% ids) }
      )
  }
