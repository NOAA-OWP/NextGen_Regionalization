...
if (m1 != "CFE+TOPMODEL") {
  gages_calib <- gages_calib[[m1]]
} else {
  gages_calib <- unique(unlist(gages_calib))
}
...
f1 <- paste0("output/",ver_receiver,"/dist_spatial_all_",calib_scenario,"_",m1,".Rdata")
if (file.exists(f1)) {
...
for (scenario in scenarios) {
  ...
  outfile <- paste0("output/",ver_receiver,"/donor_",scenario,"_",calib_scenario,"_",m1,".Rdata")
