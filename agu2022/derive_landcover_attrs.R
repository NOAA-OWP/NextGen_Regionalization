...
for (str1 in strs) {
  ...
  dt1$V1 <- round(dt1$V1, 2)
  names(dt1)[names(dt1)=="V1"] <- paste0(str1,"_nwm")
...
dt1 <- execute_zonal(rast(r2),geom=huc01,ID="id",FUN="mean",join=FALSE)
dt1$V1 <- round(dt1$V1, 2)
names(dt1)[names(dt1)=="V1"] <- "gvf_max"
attrs <- merge(attrs, dt1, by="id", all=TRUE)

dt1 <- execute_zonal(rast(r4),geom=huc01,ID="id",FUN="mean",join=FALSE)
dt1$V1 <- round(dt1$V1, 2)
names(dt1)[names(dt1)=="V1"] <- "gvf_diff"
attrs <- merge(attrs, dt1, by="id", all=TRUE)
...
dt1 <- execute_zonal(rast(r2),geom=huc01,ID="id",FUN="mean",join=FALSE)
dt1$V1 <- round(dt1$V1, 2)
names(dt1)[names(dt1)=="V1"] <- "lai_max"
attrs <- merge(attrs, dt1, by="id", all=TRUE)

dt1 <- execute_zonal(rast(r4),geom=huc01,ID="id",FUN="mean",join=FALSE)
dt1$V1 <- round(dt1$V1, 2)
names(dt1)[names(dt1)=="V1"] <- "lai_diff"
attrs <- merge(attrs, dt1, by="id", all=TRUE)
