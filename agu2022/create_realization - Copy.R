...
gages_calib <- get(load(paste0("output/",ver_donor,"/calib_gages_screened_",calib_scenario,".Rdata")))

if (model1 != "CFE+TOPMODEL") {

  gages_calib <- gages_calib[[model1]]
  
  # optimal parameters
  f1 <- paste0("data/calib_",calib_scenario,"/",tolower(model1),"_noah_kge_calib_best_params_2016_2021.csv")
  ...
  dtStats0 <- dtPars0 <- vector("list", length(mods))
  names(dtStats0) <- names(dtPars0) <- mods
  
  gages_calib <- unique(unlist(gages_calib[tolower(mods)]))
  
  for (m1 in mods) {
    # stats
    f1 <- paste0("data/calib_",calib_scenario,"/",tolower(m1),"_noah_kge_valid_stat.csv")
    dtStats0[[m1]] <- as.data.table(read.csv(f1,header=TRUE,colClasses=c("site_no"="character")))
    
    # optimal parameters
    f1 <- paste0("data/calib_",calib_scenario,"/",tolower(m1),"_noah_kge_calib_best_params_2016_2021.csv")
  ...
  for (g1 in gages_calib) {
    stat1 <- sapply(dtStats0, function(x) subset(x,site_no==g1 & run==run0 & period==period0)[[metric0]])
    mod1 <- names(stat1)[which.max(stat1)]
...
# donor pairing
dtDonorAll <- get(load(paste0("output/",ver_receiver,"/donor_",run1,"_",calib_scenario,"_",model1,".Rdata")))
...
write(toJSON(l1,digits=7,pretty=TRUE,auto_unbox=TRUE),
      paste0("realization/",model1,"_",run1,"_",calib_scenario,".json"))
}}
