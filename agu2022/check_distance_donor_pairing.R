...
huc01 <- st_read(paste0("../datasets/gpkg_",ver_receiver,"/catchment_data.geojson"))

# collection of all attributes (for donors and receivers)
dtAttrAll <- get(load(paste0("output/",ver_receiver,"/all_attrs.Rdata")))
...
for (scenario in c("hlr","camels")) {
  ...
  # donor pairing
  dtDonorAll <- get(load(paste0("output/",ver_receiver,"/donor_",scenario,"_",calib_scenario,".Rdata")))
  ...
  for (c1 in c("tag","distAttr","distSpatial")) {
    png(filename = paste0("figs/",c1,"_",scenario,"_",calib_scenario,".png"),width = 5,height=5,units="in",res=300)
  ...
  ggsave(paste0("figs/reg_diff_attr_prc_",scenario,"_",calib_scenario,".png"),gg1,width=12,height=8,units="in",dpi=300)  
  ...
  for (c1 in unique(c(attrs[["base"]],attrs[[scenario]]))) {
    ...
    png(filename = paste0("figs/reg_diff_attr_",c1,"_",scenario,"_",calib_scenario,".png"),width = 5,height=5,units="in",res=300)
  ...
  for (c1 in unique(c(attrs[["base"]],attrs[[scenario]]))) {
    ...
    png(filename=paste0("figs/density_diff_attr_",c1,"_",scenario,"_",calib_scenario,".png"),width=7.5,height=5,units="in",res=300)
